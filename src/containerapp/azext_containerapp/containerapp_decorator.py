...
from ._utils import (
    ...
    _generate_secret_volume_name,
    parse_service_bindings,
    check_unique_bindings,
    check_bindings_and_raise_error,
    delete_managed_binding,
    update_connectors_with_two_parameters,
    AppType,
...
class BaseContainerAppDecorator(BaseResource):
    ...
    def list(self):
        ...
        if managed_env:
            ...
            if "resource_group" in parse_resource_id(managed_env):
                self.get_environment_client().show(self.cmd, parse_resource_id(managed_env)[
                    "resource_group"], parse_resource_id(managed_env)["name"])
                containerapps = [c for c in containerapps if c["properties"][
                    "environmentId"].lower() == managed_env.lower()]
            else:
                containerapps = [c for c in containerapps if parse_resource_id(
                    c["properties"]["environmentId"])["name"].lower() == env_name]
    ...
    def list_secrets(self, show_values=False):
        ...
        if not containerapp_def:
            raise ResourceNotFoundError(
                "The containerapp '{}' does not exist".format(
        ...
        try:
            return self.client.list_secrets(
                cmd=self.cmd,
                resource_group_name=self.get_argument_resource_group_name(),
                name=self.get_argument_name())["value"]
        except Exception as e:
            handle_non_404_status_code_exception(e)
    ...
    def set_argument_service_connectors_def_list(
            self, service_connectors_def_list):
        self.set_param(
...
class ContainerAppCreateDecorator(BaseContainerAppDecorator):
    ...
    def validate_arguments(self):
        validate_container_app_name(
            self.get_argument_name(),
            AppType.ContainerApp.name)
        validate_create(
            self.get_argument_registry_identity(),
    ...
    def construct_payload(self):
        if self.get_argument_registry_identity() and not is_registry_msi_system(
                self.get_argument_registry_identity()):
            logger.info(
        ...
        if self.get_argument_yaml():
            return self.set_up_create_containerapp_yaml(
                name=self.get_argument_name(), file_name=self.get_argument_yaml())
        ...
        if not managed_env_info:
            raise ValidationError(
                "The environment '{}' does not exist. Specify a valid environment".format(
                    self.get_argument_managed_env()))

        while not self.get_argument_no_wait() and safe_get(
                managed_env_info,
                "properties",
                "provisioningState",
                default="").lower() in [
        ...
        if not self.get_argument_workload_profile_name(
        ) and "workloadProfiles" in managed_env_info:
        ...
        ) is not None and self.get_argument_ingress() is not None:
        ...
        registries_def = None
        if self.get_argument_registry_server() is not None and not is_registry_msi_system(
                self.get_argument_registry_identity()):
            ...
            if (self.get_argument_registry_user() is None or self.get_argument_registry_pass(
            ) is None) and self.get_argument_registry_identity() is None:
            ...
            if not self.get_argument_registry_identity():
                ...
                if secrets_def is None:
                    secrets_def = []
                registries_def["passwordSecretRef"] = store_as_secret_and_return_secret_ref(
                    secrets_def,
                    self.get_argument_registry_user(),
                    self.get_argument_registry_server(),
                    self.get_argument_registry_pass(),
        ...
        if self.get_argument_user_assigned():
            assign_user_identities = [
                x.lower() for x in self.get_argument_user_assigned()]
        else:
            assign_user_identities = []
        ...
        if self.get_argument_revision_suffix() is not None and not is_registry_msi_system(
                self.get_argument_registry_identity()):
    ...
    def create(self):
        try:
            r = self.client.create_or_update(
                ...
                resource_group_name=self.get_argument_resource_group_name(),
                name=self.get_argument_name(),
                container_app_envelope=self.containerapp_def,
    ...
    def construct_for_post_process(self, r):
        if is_registry_msi_system(self.get_argument_registry_identity()):
            ...
            safe_set(
                ...
                "configuration",
                "registries",
                value=[registries_def])

    def post_process(self, r):
        if is_registry_msi_system(self.get_argument_registry_identity()):
            r = self.create()

        if "properties" in r and "provisioningState" in r["properties"] and r["properties"]["provisioningState"].lower(
        ) == "waiting" and not self.get_argument_no_wait():
        ...
        if "configuration" in r["properties"] and "ingress" in r["properties"]["configuration"] and r[
                "properties"]["configuration"]["ingress"] and "fqdn" in r["properties"]["configuration"]["ingress"]:
            not self.get_argument_disable_warnings() and logger.warning(
    ...
    def set_up_create_containerapp_yaml(self, name, file_name):
        ...
        # Deserialize the yaml into a ContainerApp object. Need this since
        # we're not using SDK
        try:
        ...
        # Remove tags before converting from snake case to camel case, then
        # re-add tags. We don't want to change the case of the tags. Need this
        # since we're not using SDK
        tags = None
        ...
        # After deserializing, some properties may need to be moved under the
        # "properties" attribute. Need this since we're not using SDK
        ...
        # introduced in the deserialization. Need this since we're not using
        # SDK
        ...
        if self.get_argument_managed_env():
            ...
            ) and env_id is not None and env_id != self.get_argument_managed_env():
                logger.warning(
        ...
        if not env_info:
            raise ValidationError(
                "The environment '{}' in resource group '{}' was not found".format(
                    env_name, env_rg))
    ...
    def set_up_scale_rule(self):
        scale_def = None
        if self.get_argument_min_replicas(
        ) is not None or self.get_argument_max_replicas() is not None:
...
class ContainerAppUpdateDecorator(BaseContainerAppDecorator):
    ...
    def update(self):
        try:
            r = self.client.update(
                ...
                resource_group_name=self.get_argument_resource_group_name(),
                name=self.get_argument_name(),
                container_app_envelope=self.new_containerapp,
                no_wait=self.get_argument_no_wait())
            if not self.get_argument_no_wait(
            ) and "properties" in r and "provisioningState" in r["properties"] and r["properties"]["provisioningState"].lower() == "waiting":
                logger.warning(
                    'Containerapp update in progress. Please monitor the update using `az containerapp show -n {} -g {}`'.format(
                        self.get_argument_name(), self.get_argument_resource_group_name()))
            return r
        except Exception as e:
            handle_raw_exception(e)
    ...
    def set_up_from_revision(self):
        if self.get_argument_from_revision():
            r = None
            try:
                r = self.client.show_revision(
                    cmd=self.cmd,
                    resource_group_name=self.get_argument_resource_group_name(),
                    container_app_name=self.get_argument_name(),
                    name=self.get_argument_from_revision())
            except CLIError as e:
                handle_non_404_status_code_exception(e)

            _update_revision_env_secretrefs(
                r["properties"]["template"]["containers"],
                self.get_argument_name())
            safe_set(
                self.new_containerapp,
                "properties",
                "template",
                value=r["properties"]["template"])

    def _need_update_container(self):
        return self.get_argument_image() or self.get_argument_container_name() or self.get_argument_set_env_vars() is not None or self.get_argument_remove_env_vars() is not None or self.get_argument_replace_env_vars(
        ) is not None or self.get_argument_remove_all_env_vars() or self.get_argument_cpu() or self.get_argument_memory() or self.get_argument_startup_command() is not None or self.get_argument_args() is not None or self.get_argument_secret_volume_mount() is not None

    def construct_payload(self):
        # construct from yaml
        if self.get_argument_yaml():
            return self.set_up_update_containerapp_yaml(
                name=self.get_argument_name(), file_name=self.get_argument_yaml())
        ...
        # Doing this while API has bug. If env var is an empty string, API
        # doesn't return "value" even though the "value" should be an empty
        # string
        for container in safe_get(
            self.containerapp_def,
            "properties",
            "template",
            "containers",
        ...
        if update_map["container"]:
            ...
            if not self.get_argument_container_name():
                if len(
                        self.new_containerapp["properties"]["template"]["containers"]) == 1:
            ...
            for c in self.new_containerapp["properties"]["template"]["containers"]:
                if c["name"].lower() == self.get_argument_container_name().lower():
                    ...
                    if self.get_argument_startup_command() is not None:
                        if isinstance(
                                self.get_argument_startup_command(),
                                list) and not self.get_argument_startup_command():
                            c["command"] = None
                        else:
                            c["command"] = self.get_argument_startup_command()
                    if self.get_argument_args() is not None:
                        if isinstance(
                                self.get_argument_args(),
                                list) and not self.get_argument_args():
                            c["args"] = None
                        else:
                    ...
                    if self.get_argument_secret_volume_mount() is not None:
                        ...
                        if "volumeMounts" not in c or not c["volumeMounts"]:
                            # if no volume mount exists, create a new volume
                            # and then mount
                            volume_def = VolumeModel
                            volume_mount_def = VolumeMountModel
                            volume_def["name"] = _generate_secret_volume_name()
                            ...
                            if "volumes" not in self.new_containerapp["properties"][
                                    "template"] or self.new_containerapp["properties"]["template"]["volumes"] is None:
            ...
            if not updating_existing_container:
                ...
                if self.get_argument_remove_env_vars() is not None:
                    ...
                    _remove_env_vars(
                        container_def["env"],
                        self.get_argument_remove_env_vars())
        ...
        if update_map["registry"]:
            ...
            if "secrets" in self.containerapp_def["properties"][
                    "configuration"] and self.containerapp_def["properties"]["configuration"]["secrets"]:
                self.new_containerapp["properties"]["configuration"][
            ...
            if self.get_argument_registry_server():
                ...
                for r in registries_def:
                    if r['server'].lower(
                    ) == self.get_argument_registry_server().lower():
    ...
    def set_up_update_containerapp_yaml(self, name, file_name):
        if self.get_argument_image() or self.get_argument_min_replicas() or self.get_argument_max_replicas() or self.get_argument_set_env_vars() or self.get_argument_remove_env_vars() or self.get_argument_replace_env_vars(
        ) or self.get_argument_remove_all_env_vars() or self.get_argument_cpu() or self.get_argument_memory() or self.get_argument_startup_command() or self.get_argument_args() or self.get_argument_tags():
            logger.warning(
                'Additional flags were passed along with --yaml. These flags will be ignored, and the configuration defined in the yaml will be used instead')
        yaml_containerapp = process_loaded_yaml(load_yaml_file(file_name))
        if not isinstance(
        ...
        # Remove the environmentId in the PATCH payload if it has not been
        # changed
        if safe_get(
                self.new_containerapp,
                "properties",
                "environmentId") and safe_get(
                self.new_containerapp,
                "properties",
                "environmentId").lower() == existed_environment_id.lower():
...
class ContainerAppPreviewCreateDecorator(ContainerAppCreateDecorator):
    ...
    def validate_arguments(self):
        ...
        validate_create(
            ...
            self.get_argument_repo(),
            self.get_argument_yaml(),
            self.get_argument_environment_type())
    ...
    def set_up_source(self):
        if self.get_argument_source():
            ...
            # Uses buildpacks or an ACR Task to generate image if Dockerfile
            # was not provided by the user
            app.run_acr_build(
                dockerfile,
                self.get_argument_source(),
                quiet=False,
                build_from_source=not has_dockerfile)
    ...
    def set_up_repo(self):
        if self.get_argument_repo():
            # Check if container app exists and if it does, set the image to
            # the image of the existing container app
            ...
            if existing_container_app:
                ...
                if containers_from_existing_container_app is None or len(
                        containers_from_existing_container_app) == 0:
                    raise ValidationError(
                        "The container app '{}' does not have any containers. Please use --image to set the image for the container app".format(
                            self.get_argument_name()))
    ...
    def _post_process_for_repo(self):
        ...
        _create_github_action(
            app,
            env,
            self.get_argument_service_principal_client_id(),
            self.get_argument_service_principal_client_secret(),
            self.get_argument_service_principal_tenant_id(),
            self.get_argument_branch(),
            token,
            self.get_argument_repo(),
            self.get_argument_context_path())
    ...
    def _construct_app_and_env_for_source_or_repo(self):
        ...
        if parsed_env.get('resource_type').lower(
        ) == CONNECTED_ENVIRONMENT_RESOURCE_TYPE.lower():
            raise ValidationError(
        ...
        # Set image to None if it was previously set to the default image (case
        # where image was not provided by the user) else reformat it
    ...
    def post_process(self, r):
        ...
        if self.get_argument_service_connectors_def_list() is not None:
            ...
            for item in self.get_argument_service_connectors_def_list():
                while r is not None and r["properties"]["provisioningState"].lower(
                ) == "inprogress":
                    r = self.client.show(
                        self.cmd,
                        self.get_argument_resource_group_name(),
                ...
                # Case: Kafka on confluent cloud with two linker operations.
                # Validate uniqueness across all bindings
                if any(len(item["parameters"]) ==
                       2 for item in service_connectors_def_list):
                    updated_service_connectors = update_connectors_with_two_parameters(
                        item, service_connectors_def_list.copy(), r["id"])
                    check_bindings_and_raise_error(
                        self.cmd,
                        updated_service_connectors,
    ...
    def set_up_service_binds(self):
        if self.get_argument_service_bindings() is not None:
            ...
            safe_set(
                ...
                "template",
                "serviceBinds",
                value=service_bindings_def_list)
...
class ContainerAppPreviewUpdateDecorator(ContainerAppUpdateDecorator):
    ...
    def _update_container_app_source(
            self,
            cmd,
            registry_server,
            registry_user,
        ...
        # Uses buildpacks or an ACR Task to generate image if Dockerfile was
    ...
    def post_process(self, r):
        ...
        if unbind_service_bindings:
            ...
            for item in unbind_service_bindings:
                ...
                for managed_binding in managed_bindings:
                    ...
                    if binding_name in {
                            kafka_bootstrap_binding,
                            kafka_registry_binding,
                            item}:
                        delete_managed_binding(
        ...
        if self.get_argument_service_connectors_def_list() is not None:
            ...
            for item in self.get_argument_service_connectors_def_list():
                ...
                # Case: Kafka on Confluent Cloud. Boostrap server and Schema
                # registry links are created.
                else:
                    parameters_bootstrap_server, parameters_schema_registry = item["linker_name"].split(
                        '.', 1)
                    linker_create_or_update(
    ...
    def set_up_unbind_service_bindings(self):
        if self.get_argument_unbind_service_bindings():
            ...
            if new_template["serviceBinds"]:
                service_bindings_dict = {
                    service_binding["name"]: index for index,
                    service_binding in enumerate(
                        new_template.get(
                            "serviceBinds",
                            []))}
            ...
            for item in self.get_argument_unbind_service_bindings():
                if item in service_bindings_dict:
                    new_template["serviceBinds"] = [
                        binding for binding in new_template["serviceBinds"] if binding["name"] != item]
