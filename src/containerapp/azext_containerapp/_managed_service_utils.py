...
from azure.cli.core.azclierror import (
    ValidationError, InvalidArgumentValueError)
...
class ManagedRedisUtils:
    ...
    @staticmethod
    def build_redis_service_connector_def(
            subscription_id,
            resource_group_name,
            service_name,
            arg_dict,
            name,
            binding_name):
        resource_id = ManagedRedisUtils.build_redis_resource_id(
            subscription_id, resource_group_name, service_name, arg_dict)
        parameters = ManagedRedisUtils.build_redis_params(
            resource_id, name, key_vault_id=None)
        return {
            "linker_name": binding_name,
...
class ManagedCosmosDBUtils:
    ...
    @staticmethod
    def build_cosmosdb_service_connector_def(
            subscription_id,
            resource_group_name,
            service_name,
            arg_dict,
        ...
        resource_id = ManagedCosmosDBUtils.build_cosmos_resource_id(
            subscription_id, resource_group_name, service_name, arg_dict)
...
class ManagedPostgreSQLFlexibleUtils:
    ...
    @staticmethod
    def build_postgres_resource_id(
            subscription_id,
            resource_group_name,
            service_name,
            arg_dict):
    ...
    @staticmethod
    def build_postgresql_service_connector_def(
            subscription_id,
            resource_group_name,
            service_name,
            arg_dict,
            ...
            binding_name):
        if not all(
            key in arg_dict for key in [
                "database",
                "username",
                "password"]):
            raise ValidationError(
                "Managed PostgreSQL Flexible Server needs the database, username, and password arguments.")
        resource_id = ManagedPostgreSQLFlexibleUtils.build_postgres_resource_id(
            subscription_id, resource_group_name, service_name, arg_dict)
        parameters = ManagedPostgreSQLFlexibleUtils.build_postgres_params(
            resource_id, name, arg_dict["username"], arg_dict["password"], key_vault_id=None)
        return {
            "linker_name": binding_name,
            "parameters": parameters,
            "resource_id": resource_id}
...
class ManagedMySQLFlexibleUtils:
    ...
    @staticmethod
    def build_mysql_service_connector_def(
            subscription_id,
            resource_group_name,
            service_name,
            arg_dict,
        ...
        resource_id = ManagedMySQLFlexibleUtils.build_mysql_resource_id(
            subscription_id, resource_group_name, service_name, arg_dict)
        parameters = ManagedMySQLFlexibleUtils.build_mysql_params(
            resource_id, name, arg_dict["username"], arg_dict["password"], key_vault_id=None)
        return {
            "linker_name": binding_name,
            "parameters": parameters,
...
class ManagedKafkaUtils:
    ...
    @staticmethod
    def build_kafka_registry_params(
            capp_name,
            arg_dict,
            key_vault_id,
            client_type=None,
    ...
    def build_kafka_service_binding_name(binding_name, arg_dict):
        ...
        if '.' in binding_prefix:
            logger.warning(
                "Kafka on Confluent cloud binding names cannot contain periods ('.'). Removing '.' and generating binding name ...")
    ...
    def build_kafka_service_connector_def(arg_dict, name, binding_name):
        logger = get_logger(__name__)
        has_server_params = all(
            key in arg_dict for key in [
                "bootstrap_server",
                "kafka_key",
                "kafka_secret"])
        partial_registry_params = any(
            key in arg_dict for key in [
                "schema_registry",
                "schema_secret",
                "schema_secret"])
        has_registry_params = all(
            key in arg_dict for key in [
                "schema_registry",
                "schema_secret",
                "schema_secret"])
        ...
        if not has_server_params:
            logger.warning(
                "With no space in-between, Managed Kafka bootstrap server arguments are in the form:"
        ...
        if partial_registry_params and not has_registry_params:
            logger.warning(
                "With no space in-between, Managed Kafka schema registry arguments are in the form:"
        ...
        if has_server_params and has_registry_params:
            if len(arg_dict) > 6:
                logger.warning(
                    "More than the required arguments were provided. Only required arguments will be used. Proceeding with operation ...")
